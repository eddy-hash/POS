...
export class DashboardService {
  ...
  async getStats(userId: number, displayCurrency: string = 'TZS') {
    ...
    const totalProducts = await this.productRepository.count({ where: { userId, isActive: true } });
    ...
    const convertedRevenue = this.roundToTwo(this.currencyService.convert(totalRevenue, 'TZS', displayCurrency));
    const convertedExpenses = this.roundToTwo(this.currencyService.convert(totalExpenses, 'TZS', displayCurrency));
    const convertedProfit = this.roundToTwo(this.currencyService.convert(profit, 'TZS', displayCurrency));
  ...
  private async getSalesTrend(userId: number) {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      trend.push({
        date: key,
        amount: this.roundToTwo(dateMap.get(key) || 0),
      });
    }
    return trend;
  }
  ...
  private async getTopProducts(userId: number) {
    ...
    return Array.from(productMap.values())
      .map(item => ({
        ...item,
        sales: this.roundToTwo(item.sales),
      }))
