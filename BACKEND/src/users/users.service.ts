...
export class UsersService {
  ...
  async create(userData: { email: string; password: string; name: string; role_id?: number }): Promise<User> {
  ...
  async isUserLocked(userId: number): Promise<{ locked: boolean; message?: string }> {
    const user = await this.findById(userId);
    if (!user) return { locked: false };

    if (!user.isLocked) return { locked: false };
    ...
    return {
      locked: true,
      message: `You have been blocked. Please contact admin. (Lock expires in ${minutesRemaining} minutes)`,
    };
  }

  isSuperAdmin(email: string): boolean {
    return email.endsWith('@uuuu');
  }
