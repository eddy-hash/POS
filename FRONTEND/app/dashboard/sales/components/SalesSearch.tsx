...
export function SalesSearch({ search, setSearch, totalResults, totalItems }: SalesSearchProps) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
      ...
      <div className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 whitespace-nowrap">
        ...
        <span className="font-medium text-slate-700 dark:text-slate-300">{totalItems}</span> sales
      </div>
    </div>
  );
}
